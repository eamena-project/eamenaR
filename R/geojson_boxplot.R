...
#' @description
#'
#' @param stat.name the name of the output file. By default "boxplot".
...
#'
#' # Plotly plot
#' geojson_boxplot(plotly.plot = T, by = "route")
#'
#' @export
geojson_boxplot <- function(stat.name = "boxplot",
                            concept.name = "hp.id",
  ...
  if("area" %in% stat){
    ...
    for (measurements.type in lmeasurements.types){
      ...
      if(measurements.type == "Area"){
        ...
        if(!plotly.plot){
          gout <- gout +
            ggplot2::geom_jitter(ggplot2::aes(color = df.measure.type[["by"]]),
