#' For a given Heritage Place, find all related resources, select the components have a given value in a certain field
#'
#' @name select_related_resources
#'
#' @description For a given Heritage Place, find all related resources (ex: a Built Component) having a a given value (ex: "Stable") in a certain field (ex: "Measurement Number"). This function is run after `list_related_resources()`.
#'
...
#' @param having a vector of values to only keep rr having these values. By default c("Stable"). These values must appear in the `ids.csv` file with their `valueid` (see `python manage.py whatisthis`).
...
#'
#' @return A dataframe recordind the IDs and UUIDs of the Heritage Place and related resources with, for the latter, the selected fields
#'
#' @examples
#'
#' d <- hash::hash()
...
select_related_resources <- function(db.con = NA,
  ...
  for(rr in uuid.rr){
    ...
    # rr <- "34cfe992-c2c0-11ea-9026-02e7594ce0a0"
    ...
    for (hav in length(having.df[["uuid"]])){
      ...
      FROM tiles
      WHERE resourceinstanceid::text LIKE '${rr}'
  ...
  rr.havings <- data.frame(hp.id = character(),
                           hp.uuid = character(),
                           rr.id = character(),
                           rr.uuid = character(),
                           rr.type = character(),
  ...
  for(uuids.having.one in uuids.having.all){
    ...
    # print(df[1, "hp.id"])
    # print(df[1, "hp.uuid"])
    # print(df[df$rr.uuid == uuids.having.all, "rr.id"])
    # print(uuids.having.one)
    # print(as.numeric(measurements[[1]]))
    rr.having <- data.frame(hp.id = df[1, "hp.id"],
                            hp.uuid = df[1, "hp.uuid"],
                            rr.id = df[df$rr.uuid == uuids.having.all, "rr.id"],
                            rr.uuid = uuids.having.one,
                            rr.type = having,
                            rr.measure = as.numeric(measurements[[1]]))
    rr.havings <- rbind(rr.havings, rr.having)
  }
  if(disconn){
