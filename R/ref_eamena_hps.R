...
#'                                port = 5432)
#' d <- ref_eamena_hps(db.con = my_con,
#'                     d = d,
...
#'
#' @export
ref_eamena_hps <- function(db.con = NA,
                           d = NA,
                           stat = c("all"),
                           stat.type = c("all"),
                           stat.name = "users",
                           ...
                           verbose = TRUE){
  ###############################################
  # TODO: merge with `ref_users()`, etc
  ###############################################
  if(verbose){print("*start HPs' statistics")}
  if("hps_count" %in% stat | "all" %in% stat){
    ...
    sqll <- "
      ...
      WHERE graphid::text LIKE '34cfe98e-c2c0-11ea-9026-02e7594ce0a0'
            "
    d[["hps_count"]] <- DBI::dbGetQuery(db.con, sqll)
